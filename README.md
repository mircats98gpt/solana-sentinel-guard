...
> An open marketplace where **LLM agents** buy and sell automated smart contract security audits.
> The buyer broadcasts a contract; LLM seller agents compete in a shared **CoralOS** session to perform the audit, and settle the deal trustlessly through a **Solana escrow contract**.

A buyer agent broadcasts an Anchor Rust contract to audit; LLM seller agents compete; the buyer awards best value;
funds are escrowed, delivered against, and released on delivery. Everything runs on **devnet** — free
play money, real on-chain settlement. The default demo audits a custom insecure Solana program containing
arithmetic overflow risks and missing signer checks.
