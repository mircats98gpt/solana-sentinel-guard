...
async function main() {
  ...
  // Default to sentinel service with a vulnerability-ridden Anchor smart contract snippet
  const buyerService = env.BUYER_SERVICE ?? 'sentinel'
  const defaultRustSnippet = `use anchor_lang::prelude::*;
declare_id!("Fg6PaFpoGXkYsidMpWTK6W2BeZ7FEfcYkg476zPFsLnS");
#[program]
pub mod insecure_store {
    use super::*;
    pub fn deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
        let user_account = &mut ctx.accounts.user_account;
        user_account.balance += amount; // arithmetic overflow risk
        Ok(())
    }
    pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
        let user_account = &mut ctx.accounts.user_account;
        user_account.balance -= amount; // arithmetic underflow risk
        Ok(())
    }
}
#[derive(Accounts)]
...
pub struct Withdraw<'info> {
    #[account(mut)]
    pub user_account: Account<'info, UserAccount>,
    pub user_info: AccountInfo<'info>, // missing signer verification
}
#[account]
pub struct UserAccount {
    pub balance: u64,
}`
  const buyerArg = env.BUYER_ARG ?? defaultRustSnippet
