...
const KNOWN_SERVICES = new Set(['jupiter', 'coingecko', 'news', 'inference', 'claude', 'txline', 'sentinel'])
...
export async function deliverService(request: string): Promise<string> {
  ...
  switch (service) {
    ...
    case 'sentinel':
      return sentinelService(payload)
    default:
      return jupiterSwapQuote(payload)
  }
}
...
async function sentinelService(payload: string): Promise<string> {
  ...
  if (code.includes('+=') || code.includes('-=') || code.includes('*=')) {
    vulnerabilities.push({
      ...
      severity: "High",
      explanation: "Detected direct arithmetic operators (+=, -=, *=). In Anchor smart contracts, arithmetic should always use checked operations to prevent overflows/underflows.",
      patch: `- account.balance += amount;\n+ account.balance = account.balance.checked_add(amount).ok_or(error!(ErrorCode::Overflow))?;`
    });
  }
  ...
  if (code.includes('AccountInfo') && !code.includes('is_signer')) {
    vulnerabilities.push({
      ...
      severity: "Critical",
      explanation: "Detected usage of AccountInfo without verifying is_signer. Malicious actors could pass arbitrary accounts and bypass authentication.",
      patch: `- let user_info = &ctx.accounts.user_info;\n+ require!(ctx.accounts.user_info.is_signer, ErrorCode::MissingSignature);`
    });
  }
  
  if (code.includes('AccountInfo') && !code.includes('owner')) {
    vulnerabilities.push({
      title: "Missing Account Owner Check",
      severity: "Critical",
      explanation: "Detected AccountInfo without checking the account owner matches the expected program ID, allowing fake account injection.",
      patch: `- let vault = &ctx.accounts.vault;\n+ require_keys_eq!(*ctx.accounts.vault.owner, ctx.program_id, ErrorCode::InvalidOwner);`
    });
  }
  
  const hasVulnerabilities = vulnerabilities.length > 0;
  
  // Call Claude if key is set
  const key = process.env.ANTHROPIC_API_KEY;
  let responseObj: any = {
    service: "solana-sentinel-guard",
    vulnerabilities: hasVulnerabilities ? vulnerabilities : [
      {
        title: "No obvious vulnerabilities detected",
        severity: "Info",
        explanation: "Static analyzer did not find basic signature/math vulnerabilities. Contract looks solid.",
        patch: ""
      }
    ],
    timestamp: new Date().toISOString()
  ...
  if (key) {
    try {
      const prompt = `You are a Senior Solana Smart Contract Security Auditor. Analyze this code for vulnerabilities, return a detailed JSON report including a 'vulnerabilities' array with 'title', 'severity' (Critical, High, Medium, Low), 'explanation', and a 'patch' field containing a git diff patch to fix it. Code:\n${code}`;
      const raw = await claudeInference(prompt);
      const llm = JSON.parse(raw);
      if (llm && llm.completion) {
        const completionJson = JSON.parse(llm.completion.trim());
        if (completionJson && Array.isArray(completionJson.vulnerabilities)) {
          responseObj.vulnerabilities = completionJson.vulnerabilities;
          responseObj.llm_verified = true;
        }
      }
    } catch (e) {
      responseObj.error = "LLM audit failed to parse, falling back to static scan.";
    }
  }
